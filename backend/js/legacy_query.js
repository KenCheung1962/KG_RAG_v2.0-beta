...
function formatMarkdownToHtml(text) {
    ...
    // CRITICAL FIX: The input has NO newlines, so we must insert newlines BEFORE headings
    // BEFORE we can use patterns that expect line-start (^)

    // Step 1: Insert newline before heading markers that don't have one
    // Use negative lookbehind to avoid creating double newlines when
    // the heading is already preceded by a newline
    formatted = formatted.replace(/(?<![\n])(.)(#{1,6} )/g, '$1\n$2');

    // Step 2: Handle heading at very start of string (when no newline was inserted)
    formatted = formatted.replace(/^(#{1,6} )/gm, '\n$1');

    // Step 3: Now apply heading patterns (they expect line-start ^)
    // Extract ONLY heading text, NOT content after the heading

    // Heading patterns with specific lookahead for sentence boundaries
    // Pattern 1: Heading. Content (period + space indicates end of heading)
    formatted = formatted.replace(/^(#{1,6})\s+([A-Z][^#\n]*?)(\. )/gm, (match, hashes, heading, sep) => {
        const hLevel = hashes.length;
        return `<h${hLevel} class="md-h${hLevel}">${hashes}${heading.trim()}</h${hLevel}>${sep}`;
    });

    // Pattern 2: Heading [Capital Word] (lookahead for capital-lowercase-space pattern)
    formatted = formatted.replace(/^(#{1,6})\s+([A-Z][^#\n]*?)(?=\s+[A-Z][a-z]+\s)/gm, (match, hashes, heading) => {
        const hLevel = hashes.length;
        return `<h${hLevel} class="md-h${hLevel}">${hashes}${heading.trim()}</h${hLevel}> `;
    });

    // Pattern 3-9: Specific sentence starters
    formatted = formatted.replace(/^(#{1,6})\s+([A-Z][^#\n]*?)(?=\s+This\s)/gm, (match, hashes, heading) => {
        const hLevel = hashes.length;
        return `<h${hLevel} class="md-h${hLevel}">${hashes}${heading.trim()}</h${hLevel}> `;
    });
    formatted = formatted.replace(/^(#{1,6})\s+([A-Z][^#\n]*?)(?=\s+It\s)/gm, (match, hashes, heading) => {
        const hLevel = hashes.length;
        return `<h${hLevel} class="md-h${hLevel}">${hashes}${heading.trim()}</h${hLevel}> `;
    });
    formatted = formatted.replace(/^(#{1,6})\s+([A-Z][^#\n]*?)(?=\s+The\s)/gm, (match, hashes, heading) => {
        const hLevel = hashes.length;
        return `<h${hLevel} class="md-h${hLevel}">${hashes}${heading.trim()}</h${hLevel}> `;
    });
    formatted = formatted.replace(/^(#{1,6})\s+([A-Z][^#\n]*?)(?=\s+For\s)/gm, (match, hashes, heading) => {
        const hLevel = hashes.length;
        return `<h${hLevel} class="md-h${hLevel}">${hashes}${heading.trim()}</h${hLevel}> `;
    });
    formatted = formatted.replace(/^(#{1,6})\s+([A-Z][^#\n]*?)(?=\s+According\s)/gm, (match, hashes, heading) => {
        const hLevel = hashes.length;
        return `<h${hLevel} class="md-h${hLevel}">${hashes}${heading.trim()}</h${hLevel}> `;
    });
    formatted = formatted.replace(/^(#{1,6})\s+([A-Z][^#\n]*?)(?=\s+Source\s)/gm, (match, hashes, heading) => {
        const hLevel = hashes.length;
        return `<h${hLevel} class="md-h${hLevel}">${hashes}${heading.trim()}</h${hLevel}> `;
    });
    formatted = formatted.replace(/^(#{1,6})\s+([A-Z][^#\n]*?)(?=\s+the\s+literature)/gm, (match, hashes, heading) => {
        const hLevel = hashes.length;
        return `<h${hLevel} class="md-h${hLevel}">${hashes}${heading.trim()}</h${hLevel}> `;
    });

    // Format reference sources - each source on a new line
    formatted = formatted.replace(/(Source\s+\d+:[^\n]+?)(?=\s+Source\s+\d+:|\s+the\s+literature:|$)/gi, '$1\n');
    formatted = formatted.replace(/(the\s+literature:[^\n]+?)(?=\s+the\s+literature:|\s+Source\s+\d+:|$)/gi, '$1\n');
    ...
    formatted = formatted.replace(/^(\d+)\. (.+)$/gm, '<span class="md-number">$1.</span> $2');
...
async function runQuery() {
    ...
    try {
        ...
        if (responseText.startsWith('Found') && responseText.includes('relevant chunks')) {
            ...
            // Format markdown to HTML for proper display
            answerText.innerHTML = formatMarkdownToHtml(responseText);
        }
...
async function queryDatabaseOnly(queryText) {
    ...
    try {
        ...
        answerDiv.innerHTML = formatMarkdownToHtml(responseText);
    } catch(e) {
        if (e.name === 'AbortError') {
