/**
 * IMPROVED FORMATTER v11 - Ultra-aggressive removal of LLM inline References
 * 
 * Issues Fixed:
 * 1. Remove LLM-generated ##References section even with newlines and numbers
 * 2. Clean output with single "Reference" section at end
 */
...
export function formatQueryResponse(text: string): string {
  ...
  // Work on a copy for extracting references
  const originalText = text;
  let cleaned = text;

  // ======================
  // STEP 1: ULTRA-AGGRESSIVE REMOVAL OF LLM INLINE REFERENCES
  // ======================
  
  // Pattern 1: Match ##References followed by ANY content until "## References" (clean section) or "Sources:" or end
  // This handles multi-line inline reference blocks
  cleaned = cleaned.replace(
    /##+\s*References?[\s\S]*?(?=\n##+\s*References?|\nSources:|Sources:|$)/i,
    ''
  );
  
  // Pattern 2: Match ##References followed immediately by number and sources (single line or multi)
  cleaned = cleaned.replace(
    /##+\s*References?\s*\n?\s*\d+[\s\S]*?(?=\n##|\nSources:|Sources:|$)/i,
    ''
  );
  
  // Pattern 3: If there's still a ##References with Source X: or "the literature:" anywhere, nuke it
  // This matches from ##References to end of text if it contains source patterns
  if (/##+\s*References?.*(?:Source\s+\d+:|the\s+literature:)/is.test(cleaned)) {
    cleaned = cleaned.replace(
      /##+\s*References?[\s\S]*/i,
      ''
    );
  ...
  // ======================
  // STEP 2: Remove source listings from body text
  // ======================
  
  // Remove "Source X: filename.txt" patterns
  ...
  // Remove stray standalone source citations in parentheses
  ...
  // ======================
  // STEP 6: Fix headings merged with numbers
  // ======================
  ...
  // ======================
  // STEP 11: Add clean "Reference" section at end
  // ======================
  ...
  // ======================
  // STEP 12: Add Sources section for verification
  // ======================
