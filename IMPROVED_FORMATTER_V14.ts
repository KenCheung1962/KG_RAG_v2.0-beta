/**
 * IMPROVED FORMATTER v14 - Remove '.' at start of paragraphs
 * 
 * Issues Fixed:
 * 1. Remove periods at start of paragraphs (caused by bad line breaks in LLM output)
 * 2. Properly join sentences that were split across lines
 * 3. Remove malformed ##References with Source X:
 * 4. Clean headings without #, max 8 words
 */
...
export function formatQueryResponse(text: string): string {
  ...
  // ======================
  // STEP 1: FIX LINE BREAKS BEFORE PERIODS
  // ======================
  
  // Join lines that have a period at the start back to previous line
  // Pattern: word\n. word -> word. word
  cleaned = cleaned.replace(/([a-zA-Z])\n\s*\.\s+/g, '$1. ');
  cleaned = cleaned.replace(/([a-zA-Z])\r?\n\.\s+/g, '$1. ');
  
  // Also handle multiple newlines before period
  cleaned = cleaned.replace(/([a-zA-Z])\n+\.\s*/g, '$1. ');

  // ======================
  // STEP 2: Remove standalone periods at line start
  // ======================
  
  // Remove lines that start with just ". "
  cleaned = cleaned.replace(/^\s*\.\s+/gm, '');
  cleaned = cleaned.replace(/\n\.\s+/g, '\n');

  // ======================
  // STEP 3: Remove malformed ##References with Source X:
  // ======================
  ...
  // Final cleanup: remove any ". " that might still appear at start after HTML conversion
