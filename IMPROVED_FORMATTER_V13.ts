/**
 * IMPROVED FORMATTER v13 - Simple rule: Remove ##References if it contains "Source"
 * 
 * KEEP: ## References with numbered list (1. filename, 2. filename)
 * REMOVE: ##References with "Source X:" format
 */
...
export function formatQueryResponse(text: string): string {
  ...
  // ======================
  // STEP 1: REMOVE ##References sections that contain "Source"
  // ======================
  
  // Simple rule: If ##References contains "Source", remove the whole section
  // Match from ##References until next ## or Sources: or end
  cleaned = cleaned.replace(
    /##+\s*References?[\s\S]*?Source[\s\S]*?(?=\n##+\s|\nSources:|Sources:|$)/i,
    ''
  );
  
  // Also remove standalone "Source X:" lines anywhere in text
  cleaned = cleaned.replace(/\n?\s*Source\s+\d+:\s*[^\n]+/gi, '');
  
  // Remove "the literature:" lines
  cleaned = cleaned.replace(/\n?\s*the\s+literature:\s*[^\n]+/gi, '');
  
  // Remove inline (Source X) citations
  cleaned = cleaned.replace(/\(?Source\s+\d+\)?/gi, '');
  
  // Replace "the literature" text
  ...
  // ======================
  // STEP 5: Remove ". " from start of content
  // ======================
  ...
  // ======================
  // STEP 8: Convert headings to HTML - MAX 8 WORDS
  // ======================
  ...
  cleaned = cleaned.replace(/^(#{1,6})\s+(.+?)$/gm, (match, hashes, content) => {
    ...
    // Remove References headers (we'll add our own styled one)
