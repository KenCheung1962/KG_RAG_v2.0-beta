/**
 * IMPROVED FORMATTER v9 - Fix References header, remove leading ". " from content
 * 
 * Issues Fixed:
 * 1. "## References" -> "Reference" (bold, bigger font, no ##)
 * 2. Remove ". " from start of all paragraphs/content
 * 3. No # in headings, max 8 words per heading
 */
...
export function formatQueryResponse(text: string): string {
  ...
  // ======================
  // STEP 1: Clean up stray characters
  // ======================
  ...
  // ======================
  // STEP 3: Remove malformed inline References section
  // ======================
  ...
  // ======================
  // STEP 6: CRITICAL FIX - Remove ". " from start of content
  // ======================
  
  // Remove ". " that appears at start of lines (after newlines)
  cleaned = cleaned.replace(/\n\.\s+/g, '\n');
  cleaned = cleaned.replace(/^\.\s+/gm, '');
  
  // Also handle when it follows HTML tags
  ...
  cleaned = cleaned.replace(/^(#{1,6})\s+(.+?)$/gm, (match, hashes, content) => {
    ...
    // Skip References headers - we'll format it specially below
    if (/^References?$/i.test(cleanContent)) {
      return ''; // Remove it, we'll add our own
    }
  ...
  // Double-check: remove any remaining ". " at start after formatting
  ...
  // ======================
  // STEP 10: Add Reference section (singular, bold, no #)
  // ======================
  
  const uniqueRefs = references.filter(r => r && r.length > 3);
  
  if (uniqueRefs.length > 0) {
    // Use query-h1 for Reference section (bigger font, bold)
...
// Main Reference header (singular, bigger, bold)
