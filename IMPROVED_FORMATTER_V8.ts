/**
 * IMPROVED FORMATTER v8 - No # in headings, max 8 words per heading
 * 
 * Issues Fixed:
 * 1. Headings should NOT have # or ## characters
 * 2. Headings max 8 words (truncate if longer)
 * 3. Content should not start with "."
 * 4. Keep clean References section
 */
...
export function formatQueryResponse(text: string): string {
  ...
  // ======================
  // STEP 7: Convert headings to HTML - MAX 8 WORDS, NO #
  // ======================
  ...
  // ======================
  // STEP 9: Format References section
  // ======================
  
  cleaned = cleaned.replace(/##+\s*References?\s*<br>/i, 
    '<br><br><query-h2 class="query-h2 references-header">📚 References</query-h2><br><br>');
  
  cleaned = cleaned.replace(/(\d+)\.\s+([^<\n]+?\.(?:txt|pdf|docx?|md|json))/gi, 
    '<div class="reference-item"><span class="ref-number">$1.</span> $2</div>');

  // ======================
  // STEP 10: Add Sources section
  // ======================
